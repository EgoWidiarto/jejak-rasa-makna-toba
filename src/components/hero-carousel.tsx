...
const slides = [
  ...
  },
  {
    ...
    altEn: "Display of Batak Toba culinary as part of flavor heritage",
  },
  {
    src: "/images/hero/img-hero-4.png",
    alt: "Hidangan Batak Toba yang menggugah selera dalam konteks budaya",
    altEn: "Appetizing Batak Toba dishes in a cultural context",
  },
];
      ...
      <div className="overflow-hidden rounded-3xl bg-zinc-100 shadow-[0_18px_50px_rgba(0,0,0,0.12)]">
        <div className="relative aspect-video w-full">
          {slides.map((slide, index) => {
            ...
            return (
              <div key={slide.src} aria-hidden={index !== activeIndex} className={`absolute inset-0 transition-opacity duration-700 ease-in-out ${index === activeIndex ? "opacity-100" : "opacity-0"}`}>
                <Image src={slide.src} alt={altText} fill priority={index === initialIndex} loading={index === initialIndex ? "eager" : "lazy"} sizes="(max-width: 1024px) 100vw, 1280px" className="object-cover" />
          ...
          <div className="absolute left-12 top-14 z-10 max-w-[80%] text-white drop-shadow-[0_2px_8px_rgba(0,0,0,0.7)] sm:left-10 sm:top-12">
            <p className="text-base sm:text-lg md:text-2xl tracking-[0.08em] font-roboto-bold font-bold sm:text-base">
              {t("heroTitle")}
            </p>
            <h1 className="mt-0 text-4xl sm:text-5xl md:text-6xl lg:text-7xl italic leading-none font-atziluth">
              {t("heroSubtitle")}
            </h1>
          </div>

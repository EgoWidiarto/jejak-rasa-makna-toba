...
export function DishCarousel({ direction = "rtl", items, section = "tradition-dishes" }: { direction?: "ltr" | "rtl"; items?: DishItem[]; section?: "tradition-dishes" | "daily-dishes" }) {
        ...
        {[0, 1].map((group) => (
          <div key={group} className="flex shrink-0 items-stretch gap-4 pr-4 sm:gap-5 sm:pr-5">
            {itemsList.map((dish, index) => (
              <Link key={`${group}-${dish.title}`} href={`/${section}/${encodeURIComponent(dish.title)}`}>
                <article className="relative h-82 w-60 shrink-0 cursor-pointer self-end rounded-[28px] border border-black/10 bg-white px-5 pb-6 pt-16 shadow-[0_10px_30px_rgba(0,0,0,0.08)] transition-all duration-200 hover:mx-3 hover:scale-104 hover:z-9999 origin-bottom sm:h-103 sm:w-70 sm:px-6 sm:pb-6 sm:pt-16">
